...
docker_build_image() {
    if ! docker image inspect $AZP_DOCKER_IMAGE_NAME > /dev/null 2>&1; then
        ...
        docker build -t $AZP_DOCKER_IMAGE_NAME .
    else
...
docker_create_container() {
    ...
    if [[ -z "$running_container_id" ]]; then
        echo "Running agent's container..."

        docker container create -e AZP_URL="$AZP_URL" \
        ...
        echo "Some container agent is running already, skipping docker run..."
